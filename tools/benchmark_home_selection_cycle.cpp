...
int main(int argc, char** argv) {
    ...
    state.setRow(7);
    std::vector<JellyfinHomeRow> reordered = rows;
    std::rotate(reordered.begin(), reordered.begin() + 7, reordered.end());

    size_t checksum = 0;
    const auto started = std::chrono::steady_clock::now();
    for (int iteration = 0; iteration < iterations; ++iteration) {
        const auto snapshot = state.snapshot(rows);
        const auto plan = HomeScreenState::restorePlan(snapshot, reordered);
        checksum += snapshot.selectedItemByRow.size() + plan.selections.size() + static_cast<size_t>(plan.focusedRow + 1);
    }
