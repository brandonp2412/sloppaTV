...
static std::array<JellyfinItem, 5> makeItems() {
    ...
    for (size_t i = 0; i < items.size(); ++i) {
        items[i].seriesName = "A Television Series With A Moderately Long Name " + std::to_string(i);
        items[i].parentIndexNumber = 2 + static_cast<int>(i);
        items[i].indexNumber = 10 + static_cast<int>(i);
    }
    return items;
}
...
__attribute__((noinline)) static size_t baseline(const std::array<JellyfinItem, 5>& items) {
    size_t checksum = 0;
    for (const auto& item : items) checksum += episodeLabel(item).size();
    return checksum;
}
