...
std::string makeAss(int cueCount) {
    ...
    for (int index = 0; index < cueCount; ++index) {
        ...
        if (second + 1 < 10) input += '0';
        input += std::to_string(second + 1) + ".00,Default,,0,0,0,,{\\an8}<i>Subtitle line " + std::to_string(index) +
                 "</i>\\Nwith representative dialogue, and comma\n";
    }
    return input;
}

template <typename Parse> double benchmark(const std::string& input, int iterations, Parse parse, size_t& parsedCues) {
...
int main(int argc, char** argv) {
    ...
    size_t parsedCues = 0;
    const double srtMs = benchmark(
        srt, iterations, [](const std::string& input) { return parseTextSubtitleCues(input, "srt"); }, parsedCues);
    const double plainSrtMs = benchmark(
        plainSrt, iterations, [](const std::string& input) { return parseTextSubtitleCues(input, "srt"); }, parsedCues);
    const double assMs = benchmark(
        ass, iterations, [](const std::string& input) { return parseTextSubtitleCues(input, "ass"); }, parsedCues);
    std::cout << std::fixed << std::setprecision(3) << "cues=" << cueCount << " iterations=" << iterations
              << " srt_ms=" << srtMs << " plain_srt_ms=" << plainSrtMs << " ass_ms=" << assMs
              << " total_ms=" << (srtMs + plainSrtMs + assMs) << " parsed=" << parsedCues << '\n';
