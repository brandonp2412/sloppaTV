...
Verified on the physical Google TV Streamer at the app's 1920x1080 render target. That historical pass used the temporary `nz.presley.sloppatv.test` package; current development and acceptance have since been collapsed back to the single `nz.presley.sloppatv` deployment, updated in place without clearing app data:
...
- [~] Production release signing is environment-configurable and never silently falls back to a debug key. Debug/Benchmark/Release now share the single `nz.presley.sloppatv` application ID; Debug/Benchmark use the configured signing key when present or Gradle's current local debug key otherwise. Device deployment is explicitly in-place only: signature mismatches must be diagnosed and must never be bypassed by uninstalling or clearing app data. The 2026-09-03 Waydroid update found an older installed debug certificate, located the matching historical Glass debug keystore and updated the same package successfully without resetting its saved Jellyfin session. A real production-key signing pass remains pending
