...
- 2026-09-04: LAN discovery is blocked by Test Server infrastructure rather than sloppaTV parsing. Jellyfin
...
- 2026-09-04: saved-server/profile lifecycle acceptance is physically complete without touching the missing
  `secondary-test-user` user. The existing `jellyfin` account was Quick-Connected through the still-valid alternate
  `https://arr.example.invalid/jellyfin` endpoint, producing two saved endpoint profiles. Switching back to the
  ...
  restored to exactly one `jellyfin` profile at `https://jellyfin.example.invalid`. Final app audit was clean.
...
- 2026-09-04: Glass's production signing material was recovered onto Test Server after verifying its
...
- 2026-09-04: Settings acceptance exercised and restored AUTO/LARGE/EXTRA LARGE buffers, direct
  ...
  user switch. A later physical check found both saved `secondary-test-user` and `jellyfin` profiles present on
  the canonical `https://jellyfin.example.invalid` endpoint. Switching `secondary-test-user` -> `jellyfin` rendered
  the distinct `jellyfin` Home and avatar in 178 ms primary-row readiness; switching back to
  `secondary-test-user` restored its distinct Home in 173 ms. Both transitions completed without a sloppaTV
  fatal/native-signal/ANR finding, and the original `secondary-test-user` session was restored afterward.
...
- 2026-09-05: Login, Quick Connect, discovery and saved-profile focus now live together in `AccountScreenState` instead of
  ...
  login action row to SAVED USERS, and restoring the original `secondary-test-user` session without deleting accounts or clearing app
...
- 2026-09-05: Saved Jellyfin account/session ownership now lives in `SessionRegistry` instead of a raw `SloppaApp` vector plus
  ...
  streamer acceptance opened the two saved profiles, switched from `secondary-test-user` to `jellyfin`, then restored `secondary-test-user` through the
...
- 2026-09-05: Jellyfin item/domain JSON parsing now lives in `jellyfin_item_parser.{hpp,cpp}` instead of a ~150-line private
  `JellyfinClient::parseItem()` implementation. This removes nlohmann JSON from the public `jellyfin.hpp` interface and gives item,
  artwork inheritance, actor, user-data, media-stream and trickplay parsing dedicated host coverage. The full host suite and
  optimized Release build pass. Physical streamer acceptance exercised populated Home, Collections Browse, Details and real
  DirectPlay on Brooklyn Nine-Nine `48 Hours`; Media3 startup and the real media-segment request succeeded and playback was restored
