...
- [~] External player support now mirrors the Android TV client architecture without adding Java application code: native PackageManager discovery finds `ACTION_VIEW` `video/*` handlers, Settings persists an Internal/external-player choice, item options expose `PLAY EXTERNAL` when configured, and playback launches Jellyfin's authenticated static stream with resume/title extras plus known VLC/MX Player/mpv/Vimu conventions and a default external subtitle URL where the player API supports one. Host policy tests and the Android build pass, and CheckJNI runtime discovery is clean on the physical streamer; that device currently has zero compatible external video players installed, so an actual handoff remains pending. NativeActivity also has no native `onActivityResult`, so return-position/completion synchronization remains a known parity gap unless a deliberate tiny platform bridge is added.
...
- [~] Subtitle size, background and vertical-position preferences persist. Initial playback now omits `SubtitleStreamIndex` so Jellyfin can apply the account's server-side subtitle mode/default; manual subtitle language or explicit Off is carried through the current queue/autoplay chain by language, with host policy coverage. On the physical streamer, Wonder Egg `My Priority` correctly resolved the server-default English ASS stream 4; that server subtitle transcode failed, so sloppaTV retried the same item with subtitles off and reached working DirectPlay. Dedicated manual language-carry/forced-subtitle device E2E remains pending.
...
- [~] External-player selection persists in Settings and dynamically enumerates compatible Android `video/*` handlers; the physical streamer currently has no compatible external player installed for handoff E2E
...
- [~] Two clean Test Server unsigned Release builds produced the identical SHA-256 `fbcb843ad6c88aafdd2f145f482525fcd246edbe47d9663429cdb1c9646a4367`; the check is scripted, while final production-signed reproducibility remains pending
