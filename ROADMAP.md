...
- 2026-09-04: Settings acceptance exercised and restored AUTO/LARGE/EXTRA LARGE buffers, direct
  ...
  user switch. Cleanup accidentally removed the saved `secondary-test-user` profile instead of the temporary
  account; the TV is currently usable on the saved `jellyfin` profile, but `secondary-test-user` must be
