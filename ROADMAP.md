...
- [~] GitHub Actions workflow builds/tests Debug, Benchmark and Release with the pinned SDK/NDK/CMake toolchain and uploads APKs; current push runs remain green. Physical artifact installation was proven with an earlier benchmark artifact, but CI's runner-generated debug certificate is ephemeral across runs, so newer `.test` artifacts cannot update an older installed CI build without data loss. Stable CI-only debug signing remains pending; production signing is separate
