...
- [~] Persisted playback buffer presets use Media3/ExoPlayer `DefaultLoadControl`: `AUTO`, `LARGE` (50–120 s), and `EXTRA LARGE` (80–240 s), matching the Android TV reference ranges; Media3 playback is now device-exercised in Waydroid, while dedicated per-preset buffer-depth acceptance remains pending
...
- [~] Memory profiling baseline complete; the Waydroid harness now records timestamped PSS/RSS/Java/native heap/CPU samples during configurable soaks, while the required long-session runs and leak interpretation are still pending
