...
- [~] Multiple authenticated servers are retained as saved session profiles and can be switched without re-entering credentials. Switching is now a hard account boundary: active internal playback is stopped/reported using the old session before its token is cleared, and all queue/autoplay/track-preference/pending-playback plus browse/search/detail caches are reset before the new server/user Home loads; multi-server device E2E is still pending
- [~] Multiple saved users are retained per server, exposed through a native Users & Servers chooser, and can be switched without password re-entry while their token remains valid; user-scoped media/navigation/player state is cleared between profiles, while device E2E is pending
- [~] Saved-profile management supports `USE`, `FORGET` and `ADD ANOTHER ACCOUNT`; expired 401 profiles are automatically removed, recently authenticated profiles move to the front, and the chooser lazily renders Jellyfin user avatars with an initial fallback. Forgetting the active profile now runs the same full session/media-state teardown instead of only blanking the token. The current `jellyfin` acceptance user has no profile image for populated avatar E2E; PIN protection is not a reference-client parity requirement because it remains an open upstream enhancement
...
- [~] ASS/SSA direct play is implemented with the Media3 libass extension, preserving styles/positioning without video burn-in; the Canvas overlay is intentionally used because upstream has documented OpenGL-overlay failures on some Android TVs. For embedded ASS, exact Jellyfin stream selection is mapped to the corresponding embedded Media3 text ordinal rather than depending on an extracted subtitle URL. Waydroid verified English ordinal 0 → Latin American Spanish ordinal 1 with libass track changes, first-frame rendering and READY. A 2026-09-03 Waydroid transcode case also reproduced `ass-media` 0.5.1 rejecting an external ASS track whose Media3 `Format.id` was null; assigning a stable subtitle configuration ID removed the failure, created the libass track, reached READY and retained moving video. A physical-TV styled-subtitle visual pass remains pending
...
- [x] Configurable Still Watching guard and prompt; autoplay threshold policy has direct host coverage, and a 2026-09-03 physical-streamer run reached the real prompt after the configured autoplay chain, showed `KEEP WATCHING` focused, and activating it started the guarded episode at 0 ms with a rendered first frame, `PLAYING` MediaSession state and moving-frame evidence. Back clears the continuation chain instead of leaking prompt state into later manual playback.
- [x] Server-native media-segment skip actions (Intro/Outro/Recap/Preview/Commercial); real Fallout S1E2 `SKIP RECAP` verified to jump to the server-provided 0:58 segment end. Seek/segment jumps also hold their requested logical position for a bounded 750 ms reconciliation window so stale pre-seek Media3 telemetry cannot immediately snap the progress UI/reporting backward while the decoder catches up
- [~] Native queue management / play-next / Series play-all is implemented and physically exercised on the Google TV Streamer: Friends Play All de-duplicated 97 raw Jellyfin episode records into 73 season/episode slots; move-down, play-next reordering, remove and media-next are verified again on the current Media3 build, whose 2026-09-03 queue showed 71 remaining entries while playback stayed live. The pass also found a stale duplicate S1E1 whose static stream is 404; Play All now HEAD-probes duplicate slots and prefers an available copy, with one final explicit post-fix S1E1 source-selection assertion still pending.
- [~] Queue-native Shuffle and Repeat Off/One/All are implemented with host policy coverage: shuffle preserves the current item and randomizes only the unplayed tail; Repeat One restarts only on natural completion; Repeat All wraps at queue end; and manual media-next remains an explicit advance. The physical streamer now verifies the queue UI/remote toggles for Shuffle, Repeat One and Repeat All on a live Friends queue; natural-completion Repeat One/All wrap behavior still needs device-time acceptance.
...
- [x] Media3 seeks stay in-place for both DirectPlay and server-streamed playback, matching the current reference ExoPlayer path and removing the old NuPlayer-era PlaybackInfo/restart round-trip on every direct seek. Waydroid verified an exact +10000 ms seek with zero new PlaybackInfo requests and zero runtime/player errors; host-tested post-seek telemetry reconciliation now rejects stale pre-seek positions until the player reaches the target tolerance or a bounded 750 ms catch-up window expires
...
Verified on the physical Google TV Streamer at the app's 1920x1080 render target. That historical pass used the temporary `nz.presley.sloppatv.test` package; current development has since moved to the single `app.sloppatv` application ID. The legacy `nz.presley.sloppatv` install is preserved during migration rather than being destructively removed:
...
- [~] Android searchable metadata and native `ACTION_SEARCH` handling route external/voice-recognizer queries into the existing Jellyfin Search screen. `singleTask` + `onNewIntent` handles SEARCH while the app is already running; both Waydroid and the physical Google TV Streamer now verify an active `ACTION_SEARCH` for `1917` queries Jellyfin and renders the real movie result without creating another native Activity. An actual microphone/voice-recognizer invocation remains pending
- [x] Native Android `MediaSession` integration publishes playback title/episode metadata, duration, position and buffering/playing/paused state. The physical streamer verifies idle Home has no registered sloppaTV session and active playback owns the system media-button session with live episode metadata/position. On 2026-09-03 real `cmd media_session dispatch pause` and `play` events were delivered by Android to sloppaTV, logged by the callback bridge, and changed system state `PLAYING → PAUSED → PLAYING`; host policy covers the remaining Stop/Seek/Next/Previous bridge commands and the session advertises only supported actions.
...
- [x] Native in-app screensaver is implemented with persisted Off/5/10/20/30-minute idle thresholds, a low-redraw clock/brand surface that changes safe position every 30 seconds, suppression during playback/loading/Quick Connect, and first-key dismissal back to the unchanged underlying screen. On the physical Google TV Streamer the 5-minute setting activated after the real idle interval, the first key returned to the unchanged Settings screen, and the test restored the user's setting to Off afterward
...
- [~] Branded Android TV vector banner and adaptive launcher/round icons replace the previous flat placeholder banner. A physical FLauncher pass exposed that the Activity was still presenting the default Android icon because only the application carried `android:banner`; the Leanback Activity now declares the banner explicitly and the fix passes host manifest tests plus hosted Android resource compilation. Physical re-install verification is pending because the currently installed benchmark package is signed by an older ephemeral CI debug certificate and Android correctly refuses an in-place update signed by a newer runner key
...
- [x] Search, Settings search and login use the configured Android TV system IME when available. Waydroid verifies the real Android LatinIME overlay and live EditText→JNI→native filter updates; on 2026-09-03 Back closed an active Search IME (`mInputShown=true → false`) while leaving the native Search screen in place, proving the key no longer leaks into native navigation before the cancel callback commits. The native fallback keyboard remains available and its left/right row-edge wraparound has policy-test coverage.
- [x] Max streaming bitrate
- [~] Persisted playback buffer presets use Media3/ExoPlayer `DefaultLoadControl`: `AUTO`, `LARGE` (50–120 s, 2.5/5 s playback/rebuffer start), and `EXTRA LARGE` (80–240 s, 5/10 s playback/rebuffer start). The exact duration mapping now lives in host-tested C++ playback policy; the Java Media3 bridge receives only the resolved durations and applies them to `DefaultLoadControl`. Dedicated on-device per-preset buffer-depth acceptance remains pending
...
- [x] Next Up autoplay behavior; repeated physical-streamer episode completions advanced automatically through the real Jellyfin series chain and into the Still Watching threshold.
- [x] Still Watching threshold; physical-streamer acceptance reached the configured guard after the autoplay chain and `KEEP WATCHING` resumed moving playback with a live MediaSession.
...
- [x] Persisted in-app screensaver timeout preference offers Off/5/10/20/30 minutes and drives the native idle saver; physically verified on the Google TV Streamer at 5 minutes: the saver appeared after the idle interval, the first key dismissed it back to the unchanged Settings screen, and the test restored the user's setting to Off afterward
...
- [x] Bounded thread pool instead of one thread per request/report; the worker boundary now catches unexpected task exceptions, reports them through the app's error callback/logcat, and continues later queued work, with a host regression test proving a throwing task does not terminate the pool
- [~] Waydroid E2E harness (`tools/waydroid_e2e.py`) requires an explicit ADB serial, validates Waydroid identity/1920x1080, captures screenshots/frame-difference motion evidence and filtered player logs, and now has reusable lifecycle/HOME restoration, runtime VIEW/SEARCH, MediaSession dump, app-scoped fatal-log audit, and PSS/RSS/CPU soak commands. Player-specific commands now require an active sloppaTV MediaSession before sending controls so a Home-screen invocation cannot create false playback evidence. The final 2026-09-03 Wonder Egg run measured moving playback (84.12 mean ROI diff), stable pause (0.00), resumed motion (104.86), and retained motion after forward/back/rapid seeks (82.21/73.99/44.19), with no sloppaTV fatal exception/native signal/ANR. The final broad codec/HDR/long-soak matrix remains in progress.
...
- [~] The final-gate sample counts have now been run on the current installed non-debuggable Benchmark candidate against official Jellyfin on the physical streamer: 20 cold launches, 5 settled-memory samples and 5 rapid-DPAD runs. sloppaTV measured 222.5 ms startup median vs 414 ms, 37.6 MB PSS vs 150.4 MB, 114.8 MB RSS vs 237.2 MB, and 16.72 ms navigation p95 vs 33.35 ms with 0.8% vs 10.3% of intervals over 20 ms. The final production-signed candidate and required long soaks remain pending
- [~] Production release signing is environment-configurable and never silently falls back to a debug key. Debug/Benchmark/Release now share the single `app.sloppatv` application ID; Debug/Benchmark use the configured signing key when present or Gradle's current local debug key otherwise. Device deployment is explicitly in-place for `app.sloppatv`: signature mismatches must be diagnosed and must never be bypassed by uninstalling or clearing app data. The previous `nz.presley.sloppatv` identity is treated as a legacy migration source and is preserved until any needed app-data/session migration has been verified. On 2026-09-03 all Android variants built with the renamed Java/JNI namespace, APK metadata reported `app.sloppatv`, and the Benchmark APK launched on Waydroid as `app.sloppatv/.SloppaNativeActivity` with clean native/JNI startup and no fatal/ANR entries. A real production-key signing pass remains pending
- [~] GitHub Actions workflow builds/tests Debug, Benchmark and Release with the pinned SDK/NDK/CMake toolchain and uploads APKs; current push runs remain green. CI artifact signing still needs a stable key if those artifacts are to update an existing device install; an incompatible certificate is treated as a deployment failure to diagnose, not a reason to remove the installed app
- [x] On 2026-09-03, the legacy `nz.presley.sloppatv` and `nz.presley.sloppatv.test` packages were explicitly removed from the selected Google TV Streamer. A new 4096-bit `app.sloppatv` release signing identity is held in the local ignored `key.properties` and keystore; Debug, Benchmark and Release were built with the identical certificate, and the signed Release APK installed and cold-launched successfully on the streamer.
...
The Test Server hardening branch adds a real navigation stack, native diagnostics/server-version checks, in-flight GET coalescing plus a short-lived API cache, watched/clock/backdrop settings, centralized versioning/changelog, production-signing configuration, a non-debuggable benchmark build type using the same application ID, host test orchestration, CI, and a repeatable release-reproducibility check. All host tests and Debug/Benchmark/Release Android builds pass locally across the configured ABIs. The first hosted push and pull-request CI runs are also green. Items that change visible Android TV behavior remain partial until the required Waydroid/target-TV acceptance pass is run.
