...
int main() {
    ...
    coordinator.selectAudioStream(3);
    coordinator.selectSubtitleStream(7);
    const auto coordinatedFallbackAttempt = coordinator.fallbackAttempt(true, 43210, true);
    assert(coordinatedFallbackAttempt);
    const auto coordinatedFallback = coordinatedFallbackAttempt->plan;
    ...
    assert(coordinatedFallbackAttempt->item.id == coordinatedItem.id);
    assert(coordinatedFallbackAttempt->item.positionTicks == coordinatedFallback.resumeTicks);
    assert(coordinatedFallbackAttempt->failedTarget.url == coordinatedFallbackTarget.url);
    assert(coordinatedFallbackAttempt->audioStreamIndex == 3);
    assert(coordinatedFallbackAttempt->subtitleStreamIndex == 7);
