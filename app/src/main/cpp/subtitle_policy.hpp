...
// nullopt: no queue override, let Jellyfin apply the user's server-side subtitle mode/default.
...
inline int subtitleIndexForQueuePreference(
    ...
    if (!languagePreference.has_value()) return kSubtitleServerDefaultIndex;
