...
public:
    ...
    bool playAll(JellyfinSession session, JellyfinItem series, SeriesPlayAllOptions options, uint64_t generation) {
        return tasks_.submit(
            [this, session = std::move(session), series = std::move(series), options, generation]() mutable {
                ...
                auto target = client_.resolvePlayback(session, first, options.maxStreamingBitrate,
                                                      options.maxAudioChannels, options.overrides);
                if (!epoch_.active(generation)) return;
                completions_.push(SeriesPlayAllCompletion{
                    .generation = generation,
                    .series = std::move(series),
