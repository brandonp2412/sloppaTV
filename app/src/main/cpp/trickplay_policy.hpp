...
struct TrickplayFrame {
    ...
    [[nodiscard]] constexpr bool valid() const { return thumbnailIndex >= 0 && tileIndex >= 0; }
};
...
struct TrickplayUvRegion {
    ...
    [[nodiscard]] constexpr bool valid() const { return u1 > u0 && v1 > v0; }
};

constexpr TrickplayFrame trickplayFrameForPosition(int64_t positionMs, int intervalMs, int thumbnailCount,
...
constexpr TrickplayUvRegion trickplayUvRegion(const TrickplayFrame& frame, int cellWidth, int cellHeight,
