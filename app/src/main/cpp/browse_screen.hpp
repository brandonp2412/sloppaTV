...
public:
    ...
    void removeItem(const std::string& itemId) {
        ...
        auto remove = [&](std::vector<JellyfinItem>& items) {
            ...
            return static_cast<int>(previousSize - items.size());
        };
        nextIndex_ = std::max(0, nextIndex_ - remove(items_));
        for (auto& snapshot : stack_) {
            snapshot.nextIndex = std::max(0, snapshot.nextIndex - remove(snapshot.items));
        }
