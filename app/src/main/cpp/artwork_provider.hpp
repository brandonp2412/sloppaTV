...
template <
    typename JellyfinLike,
    typename SeerrLike,
    typename DecoderLike,
    typename TaskRunnerLike,
    typename MutexLike
>
...
public:
    ...
    ArtworkProvider(
        JellyfinLike& jellyfin,
        SeerrLike& seerr,
        DecoderLike& decoder,
        TaskRunnerLike& tasks,
        MutexLike& stateMutex,
        HomeLoadObserver homeLoadObserver = nullptr
    )
        : loader_(jellyfin, seerr, decoder),
          coordinator_(tasks, stateMutex),
          homeLoadObserver_(homeLoadObserver) {}

    void setDataPath(std::string dataPath) {
        loader_.setDataPath(std::move(dataPath));
    }
    ...
    ArtworkEntry* posterTexture(
        ...
        return coordinator_.posterTexture(
            key,
            renderer,
            [&] { requestPoster(session, item, external, renderer); }
        );
    }
    ...
    ArtworkEntry* profileTexture(const JellyfinSession& session, RendererLike& renderer) {
        ...
        return coordinator_.profileTexture(
            key,
            renderer,
            [&] { queueProfile(session, key, renderer); }
        );
    }

    template <typename RendererLike>
    void eraseProfile(const JellyfinSession& session, RendererLike& renderer) {
    ...
    ArtworkEntry* homeTexture(
        ...
        return coordinator_.homeTexture(
            key,
            renderer,
            [&] { requestHome(session, item, external, renderer); }
        );
    }

    template <typename RendererLike>
    ArtworkEntry* backdropTexture(
        const JellyfinSession& session,
        const JellyfinItem& item,
        int backdropMode,
        RendererLike& renderer
        ...
        return coordinator_.backdropTexture(
            key,
            renderer,
            [&] {
    ...
    ArtworkEntry* logoTexture(
        ...
        return coordinator_.logoTexture(
            key,
            renderer,
            [&] {
                if (!session.valid()) return;
    ...
    void clearSession(RendererLike& renderer) {
        coordinator_.clearSession(renderer);
    }
...
private:
    ...
    bool queuePoster(
        ...
        return coordinator_.loadPoster(
            key,
            renderer,
            [this, saved, request = std::move(request)] {
                return loader_.loadPoster(saved, request);
            }
        );
    }
    ...
    bool queueProfile(
        ...
        const JellyfinSession saved = session;
        return coordinator_.loadProfile(
            key,
            renderer,
            [this, saved] { return loader_.loadProfile(saved); }
        );
    }
    ...
    bool queueHome(
        ...
        return coordinator_.loadHome(
            ...
            renderer,
            [this, saved, request = std::move(request)] {
                return loader_.loadHome(saved, request);
            },
    ...
    bool queueBackdrop(
        const JellyfinSession& session,
        BackdropArtworkRequest request,
        RendererLike& renderer
    ) {
        if (!session.valid()) return false;
        const JellyfinSession saved = session;
        const std::string key = request.key;
        return coordinator_.loadBackdrop(
            key,
            renderer,
            [this, saved, request = std::move(request)] {
    ...
    bool queueLogo(
        ...
        return coordinator_.loadLogo(
            ...
            renderer,
            [this, saved, request = std::move(request)] {
                return loader_.loadLogo(saved, request);
            }
        );
    }
